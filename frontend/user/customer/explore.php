...
$all_facilities = $conn->query("SELECT id, name, icon, category FROM facilities ORDER BY category, name")->fetch_all(MYSQLI_ASSOC);
...
foreach ($all_facilities as $f) {
  $cat = $f['category'] ?? 'common';
  if (!isset($facilities_by_category[$cat])) {
    $facilities_by_category[$cat] = [];
  }
  $facilities_by_category[$cat][] = $f;
}
if ($isLoggedIn) {
...
<head>
  ...
  <style>
    ...
    .price-range-slider {
      ...
      height: 8px;
      border-radius: 5px;
      background: linear-gradient(to right, #28a745 0%, #28a745 50%, #e9ecef 50%, #e9ecef 100%);
      outline: none;
    ...
    .filter-sidebar {
      ...
      position: sticky;
      top: 80px;
      /* Increased from 20px to give space below navbar */
    ...
    .filter-category:not(.collapsed) .filter-options {
      max-height: 800px;
      /* Diperbesar dari 500px ke 800px */
      overflow-y: auto;
      /* Tambahkan scrollbar jika masih kepanjangan */
    }
    ...
    /* Tambahkan padding untuk sub-kategori */
    .subcategory-content {
      margin-top: 8px;
      padding-bottom: 10px;
    }
    ...
    .property-badge.putra {
      color: #1976d2;
    }

    .property-badge.putri {
      color: #c2185b;
    }

    .property-badge.campur {
      color: #f57c00;
    }
...
<body>
        ...
        <div class="filter-sidebar">
            ...
            <div id="priceFilter" class="filter-options">
              <div class="price-slider-container">
                <div class="price-display mb-3">
                  <span class="text-muted" style="font-size: 0.85rem;">Maksimal Harga:</span>
                  <div class="price-value" id="priceValue" style="font-weight: 600; color: #28a745; font-size: 1.1rem;">
                    Rp <?= number_format($filter_price ?: 10000000, 0, ',', '.') ?>
                  </div>
                </div>
                ...
                <input
                  ...
                  max="10000000"
                  step="100000"
                  value="<?= $filter_price ?: 10000000 ?>"
                  style="width: 100%;">

                <div class="price-marks d-flex justify-content-between mt-2" style="font-size: 0.75rem; color: #6c757d;">
                  <span>Rp 0</span>
                  <span>Rp 5jt</span>
                  <span>Rp 10jt</span>
                </div>
                ...
                <div class="price-presets mt-3 d-flex flex-wrap gap-2">
                  <button type="button" class="btn btn-sm btn-outline-success" onclick="setPriceValue(500000)">500rb</button>
                  <button type="button" class="btn btn-sm btn-outline-success" onclick="setPriceValue(1000000)">1jt</button>
                  <button type="button" class="btn btn-sm btn-outline-success" onclick="setPriceValue(2000000)">2jt</button>
                  <button type="button" class="btn btn-sm btn-outline-success" onclick="setPriceValue(5000000)">5jt</button>
                  <button type="button" class="btn btn-sm btn-outline-success" onclick="setPriceValue(10000000)">Semua</button>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="filter-category collapsed">
            ...
            <div id="facilitiesFilter" class="filter-options">
              <?php foreach ($facilities_by_category as $cat => $facilities): ?>
                <!-- Sub-kategori -->
                <div class="facility-subcategory">
                  <div class="subcategory-header" style="cursor: pointer; display: flex; justify-content: space-between; align-items: center;" onclick="toggleSubCategory(this)">
                    <span><?= htmlspecialchars($category_labels[$cat] ?? ucfirst($cat)) ?></span>
                    <i class="bi bi-chevron-down" style="font-size: 0.9rem; transition: transform 0.2s;"></i>
                  </div>
                  <div class="subcategory-content" style="margin-top: 8px;">
                    <?php foreach ($facilities as $f): ?>
                      <div class="form-check">
                        ...
                        <label class="form-check-label" for="fac-<?= (int)$f['id'] ?>">
                          <i class="fa <?= htmlspecialchars($f['icon']) ?>"></i>
  ...
  <script>
    ...
    function applyFilters() {
      ...
      if (price && price < 10000000) urlParams.append('price', price);
    ...
    function clearAllFilters() {
      ...
      document.getElementById('priceSlider').value = 10000000;
  ...
  <script>
    // Update slider background gradient and price display
    ...
    function updateSliderBackground() {
      ...
      priceSlider.style.background = `linear-gradient(to right, #28a745 0%, #28a745 ${percentage}%, #e9ecef ${percentage}%, #e9ecef 100%)`;
    }
