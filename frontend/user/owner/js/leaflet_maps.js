...
function initMap() {
    ...
    map = L.map('map').setView([DEFAULT_LOCATION.lat, DEFAULT_LOCATION.lng], 15);
...
function setupSearch() {
    ...
    function performSearch() {
        ...
        const url = `http://localhost/Web-App/proxy_nominatim.php?endpoint=search&format=json&q=${encodeURIComponent(query)}&countrycodes=id&limit=5`;
        ...
        .catch(error => {
            ...
            searchResults.innerHTML = '<div class="search-result-item text-danger">Gagal mencari lokasi. Coba lagi.</div>';
        });
    }
    ...
    function displaySearchResults(results) {
        if (results.length === 0) {
            searchResults.innerHTML = '<div class="search-result-item">Tidak ada hasil. Coba kata kunci lain.</div>';
            return;
        }
